...
var MP4Reader = (function reader() {
  ...
  constructor.prototype = {
    ...
    readBox: function readBox(stream) {
      console.info("MP4Reader::readBox");
      var box = { offset: stream.position };
      console.log("This box has offset "+stream.position)

      function readHeader() {
        box.size = stream.readU32();
        box.type = stream.read4CC();
        console.log("This box is "+box.size+" bytes.")
      }
      ...
      readHeader();
      console.log("This box type: "+box.type)
      switch (box.type) {
        case 'ftyp':
    ...
    read: function () {
      console.log("I am starting to read a file")
    ...
    traceSamples: function () {
      console.info("MP4Reader::traceSamples");
...
var Decoder = (function reader() {
  ...
  function constructor(gl, useWorkers) {
    ...
    if (this.useWorkers) {
      ...
      this.avcWorker.onReceiveMessage("on-picture-decoded", function (message) {
          var array = new Uint8Array(message.payload);
          onPictureDecoded.call(this, array);
      }.bind(this));
      this.avcWorker.onReceiveMessage("on-size-changed", function (message) {
            this.DecodedSize = { w: message.payload.width, h: message.payload.height };
      }.bind(this));
    } else {
      this.avc = new Avc();
...
function onPictureDecoded(buffer, w , h) {
    var width = w;
    var height = h;
    if(this.DecodedSize) {
        width = this.DecodedSize.w;
        height = this.DecodedSize.h;
    }
    ...
    this.webGLCanvas.fillYUVTextures(
    ...
    this.isReady = true;
    this.cFrameTex = this.webGLCanvas.GetFrameTexture();
  }
  ...
  constructor.prototype = {
    ...
    prepareDecode: function() {
      this.webGLCanvas = new YUVRenderer(this.gl, this.size);

      this.video = this.reader.tracks[1];
      var audio = this.reader.tracks[2];

      var avc = this.video.trak.mdia.minf.stbl.stsd.avc1.avcC;
      var sps = avc.sps[0];
      var pps = avc.pps[0];
    ...
    nextFrame: function() {
      if (this.useWorkers) {
          ...
          this.video.getSampleNALUnits(this.CurrentFrame).forEach(function (nal) {
              nal = new Uint8Array(nal);
              avcWorker.sendTransverable("decode-sample", nal.buffer);
    ...
    GetFrameTexture: function() {
      if(this.isReady) {
        this.isReady = false;
        var TexID = this.cFrameTex;
        //get TextureID
...
var Broadway = (function broadway() {
  function constructor(div) {
    ...
    this.url = src;
    this.decoder = new Decoder(gl, useWorkers);

  }
  constructor.prototype = {
    play: function (decoding) {
      this.decoder.Open(this.url);
        console.info("Opening: " + this.url);

      setTimeout(function foo() {
        if(this.decoder.IsReady(0)) {
          console.info(this.decoder.GetFrameTexture());
        } else {
          console.info("waiting for mp4");
        }
